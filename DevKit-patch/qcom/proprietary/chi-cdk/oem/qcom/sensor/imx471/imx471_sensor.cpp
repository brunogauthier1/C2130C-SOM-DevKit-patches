...
#undef IMX471_DBG
#ifdef IMX471_DBG_ENABLE
#define IMX471_DBG(fmt, ...) __android_log_print(ANDROID_LOG_INFO, "IMX471",fmt, __VA_ARGS__)
#else
#define IMX471_DBG(fmt, ...)
#endif

static const UINT   IMX471MinRegisterGain        = 0;                                     ///< Minimum analog register gain
static const UINT   IMX471MaxRegisterGain        = 960;                                   ///< Maximum analog register gain
static const DOUBLE IMX471MinRealGain            = (1024 / (1024 - IMX471MinRegisterGain)); ///< Minimum analog real gain (1X)
static const DOUBLE IMX471MaxRealGain            = (1024 / (1024 - IMX471MaxRegisterGain)); ///< Maximum analog real gain (16X)
static const UINT   IMX471MinDigitalRegisterGain = 256;                                   ///< Minimum digital register gain
static const UINT   IMX471MaxDigitalRegisterGain = 256;                                   ///< Maximum digital register gain
static const UINT   IMX471DigitalGainDecimator   = 256;                                   /// < Digital gain decimator factor
static const DOUBLE IMX471MinDigitalRealGain     = (IMX471MinDigitalRegisterGain / IMX471DigitalGainDecimator);
                                                                                          ///< Minimum digital real gain (1X)
static const DOUBLE IMX471MaxDigitalRealGain     = (IMX471MaxDigitalRegisterGain/ IMX471DigitalGainDecimator);
...
static DOUBLE RegisterToRealGain(
    UINT registerGain)
{
    if(IMX471MaxRegisterGain < registerGain)
    {
        registerGain = IMX471MaxRegisterGain;
    }
...
static UINT RealToRegisterGain(
    DOUBLE realGain)
{
    if (IMX471MinRealGain > realGain)
    {
        realGain = IMX471MinRealGain;
    }
    else if (IMX471MaxRealGain < realGain)
    {
        realGain = IMX471MaxRealGain;
    }
...
{
    float digitalRealGain = IMX471MinDigitalRealGain;

    if (IMX471MaxRealGain < totalRealGain)
    {
        digitalRealGain = totalRealGain / sensorRealGain;
    }
    else
    {
        digitalRealGain = IMX471MinDigitalRealGain;
    }

    if (IMX471MaxDigitalRealGain < digitalRealGain)
    {
        digitalRealGain = IMX471MaxDigitalRealGain;
    }

    return static_cast<UINT>(digitalRealGain * IMX471DigitalGainDecimator);
}
    ...
    SensorCalculateExposureData* pCalculateExposureData)
{
    ...
    pExposureInfo->digitalRealGain      = static_cast<FLOAT>(pExposureInfo->digitalRegisterGain / IMX471DigitalGainDecimator);
